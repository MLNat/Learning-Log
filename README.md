...
July 6th: Morning 32 minutes. Finished the first course (yay) on Python basics with Numpy array exploration like median, mean etc. Began on MATPLOTLIB (Intermediate Python course) and practiced plt.show() -- not an option to just choose one to show which is surprising. Just done lineplot (plt.plot) and scatter (plt.scatter) so far (but practice for that is tomorrow!) 
